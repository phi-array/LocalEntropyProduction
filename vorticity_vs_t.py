...
def animate(i):
    ...
    vorticity = cal_vorticity(flow)
    piv = ax[1].imshow(vorticity, cmap='seismic', interpolation='none', vmin=-1, vmax=1)
    if i == 0:
        cbar = fig.colorbar(piv, cax=ax[2], extend='both')
        cbar.set_label(label=r"s$^{-1}$", size=14)
...
ani.save(filename[:-4]+"_vorticity_vs_t"+filename[-4:], writer='ffmpeg', fps=fps, dpi=300)
# plt.show()
