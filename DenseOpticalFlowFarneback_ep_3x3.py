...
filename = "1_1_60um_1mm_lattice_evolution_4x_GreenRed_SpectraIII_Cyan3_Yellow100_bright_low_res(1d25).m4v"
dt = 0.1666733  # sec
...
#    speeds.append((vel[:, :, 1] > 200))
    ...
    state_3x3 = np.zeros_like(state, dtype=int)
    h, w = state_3x3.shape
    for i, j in np.ndindex(h-2, w-2):
        I = i+1
        J = j+1
        state_3x3[I, J] = state[I-1, J-1] + state[I, J-1]*b + state[I+1, J-1]*b*b
        state_3x3[I, J] = state_3x3[I, J] + state[I-1, J]*b*b*b + state[I, J]*b*b*b*b + state[I+1, J]*b*b*b*b*b
        state_3x3[I, J] = state_3x3[I, J] + state[I-1, J+1]*b*b*b*b*b*b + state[I, J+1]*b*b*b*b*b*b*b + state[I+1, J+1]*b*b*b*b*b*b*b*b

    return state_3x3[1:-1, 1:-1]
